#!/usr/bin/env python3
"""
Maximum thermal stress — batch=8 inference for 5 minutes.
"""
    ...
    def read_gpu_util():
        ...
        for p in paths:
            try:
                ...
                return val / 10  # 0-1000 → 0-100%
            except (IOError, ValueError):
                continue
        return 0.0

    # Main stress loop
    print(f"\nStarting {DURATION_S}s thermal stress "
          f"(batch={BATCH_SIZE})...")
    print(f"{'Time':>6s} {'GPU°C':>7s} {'CPU°C':>7s} "
          f"{'TJ°C':>7s} {'Power':>7s} {'GPU%':>6s} "
          f"{'Infs':>8s}")
    print("-" * 60)
    ...
    while True:
        ...
        if time.monotonic() - last_log >= 5.0:
            ...
            print(f"{elapsed:>5.0f}s {gpu_t:>6.1f} {cpu_t:>6.1f} "
                  f"{tj_t:>6.1f} {power:>6.1f}W {gpu_util:>5.0f}% "
