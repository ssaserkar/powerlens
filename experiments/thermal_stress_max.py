#!/usr/bin/env python3
"""
Maximum stress — batch=32 GPU + CPU stress for 5 minutes.
"""
...
def cpu_stress_worker(stop_event):
    """Burn all CPU cores with matrix multiply."""
...
def main():
    ...
    try:
        engine = build_engine_for_batch_size(MODEL, BATCH_SIZE)
    except RuntimeError:
        print("Batch=32 failed (likely OOM), trying batch=16...")
        BATCH_SIZE = 16
        engine = build_engine_for_batch_size(MODEL, BATCH_SIZE)
    ...
    for i in range(engine.num_io_tensors):
        ...
        ptr = ctypes.c_void_p()
        ret = cuda.cudaMalloc(ctypes.byref(ptr), host_data.nbytes)
        if ret != 0:
            print(f"cudaMalloc failed for {name} "
                  f"(shape={shape}, {host_data.nbytes} bytes)")
    ...
    # Main stress loop
    print(f"\nStarting {DURATION_S}s MAXIMUM stress "
          f"(GPU batch={BATCH_SIZE} + 6 CPU threads)...")
    ...
    out = Path("experiments/results/thermal_stress_max.json")
